...
namespace xmlpl {
  ...
  xmlEntitiesTablePtr entitiesTable = 0;
  ...
  entityDesc entityArray[] = {
	...
	static xmlEntityPtr nullEntity;

  void initEntitiesTable() {
    if (entitiesTable) return;
    entitiesTable = xmlCreateEntitiesTable();

		nullEntity = (xmlEntityPtr)xmlMalloc(sizeof(xmlEntity));
		memset(nullEntity, 0, sizeof(xmlEntity));
		nullEntity->type = XML_ENTITY_DECL;
		nullEntity->name = xmlStrdup((const xmlChar *)"");
		nullEntity->etype = (xmlEntityType)XML_INTERNAL_PREDEFINED_ENTITY;
		nullEntity->length = 0;
		nullEntity->content = xmlStrdup((const xmlChar *)"");
		

    for (unsigned int i = 0;
				 i < sizeof(entityArray) / sizeof(entityArray[0]); i++) {

      xmlEntityPtr e = (xmlEntityPtr)xmlMalloc(sizeof(xmlEntity));
      memset(e, 0, sizeof(xmlEntity));
      e->type = XML_ENTITY_DECL;
    
      e->name = xmlStrdup((const xmlChar *)entityArray[i].name);
      e->etype = (xmlEntityType)XML_INTERNAL_PREDEFINED_ENTITY;

      xmlChar buf[5];
      int j = 0;
      unsigned int c = entityArray[i].value;
      int bits;
      ...
      if (c < 0x80) {
				...
				buf[j++] = ((c >> 12) & 0x0F) | 0xE0;
				bits = 6;

      } else {                
				buf[j++] = ((c >> 18) & 0x07) | 0xF0;
				bits = 12;
      }
    
      for (; bits >= 0; bits -= 6)
				buf[j++] = ((c >> bits) & 0x3F) | 0x80;

      buf[j] = 0;

      e->length = j;
      e->content = xmlStrndup(buf, j);
      
      if (xmlHashAddEntry(entitiesTable, (const xmlChar *)entityArray[i].name,
													e))
				xmlFree(e);
    }
  }
  ...
  xmlEntityPtr lookupEntity(const xmlChar *name) {
    if (!entitiesTable) initEntitiesTable();
    xmlEntityPtr e = (xmlEntityPtr)xmlHashLookup(entitiesTable, name);
		//if (!e) return nullEntity;
		//else
		return e;
  }
};
